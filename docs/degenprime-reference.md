...
| `summary` | read-only | Degen Account assets / debts + **live solvency** (health ratio, total value, debt, solvent flag) via RedStone-gated SolvencyFacet reads (falls back to balances-only if the gateway is down). |
