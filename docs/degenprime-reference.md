...
Avalanche's *transaction* gas-price floor in the tool is **1 gwei** (legacy gas pricing on the C-chain, post-Etna ACP-125 minimum is 1 nAVAX); its GMX exec-fee estimator floors at 1 gwei separately. Base has no GMX integration and no such requirement; the tool uses **`max(network_price * 2, 1 gwei)`** to keep txs from stranding when Base's ~0.001 gwei base fee ticks up after submission. Cost is negligible on Base.
