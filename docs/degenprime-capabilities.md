...
- Build flow:
  1. `_paraswap_price_route(srcToken, srcDec, destToken, destDec, amountIn, userAddress=DegenAccount)` → `GET /prices` on `apiv5.paraswap.io` with `network=8453`, `version=6.2`, `side=SELL`, `includeContractMethods=swapExactAmountIn,swapExactAmountInOnUniswapV3` (an allowlist pinning the API to exactly the two facet-decodable routes).
