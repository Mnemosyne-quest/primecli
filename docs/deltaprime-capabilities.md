...
**Execution-fee gas-price floor (real bug, fixed 24-05-2026).** Avalanche's live base fee can be ~0.01–0.02 gwei, which estimates a uselessly tiny fee the keeper rejects (the request expires and refunds without minting). The tool floors the gas price at **1 gwei** in the fee estimator and pads by a 2× buffer multiplier so the `msg.value` clears GMX's requirement (~0.08–0.19 AVAX) even if gas rises before the keeper executes. The EOA must hold the execution fee up front, on top of the deposit amount and its own tx gas.
