...
def get_w3():
    ...
    _proxy_rpc = "127.0.0.1" in ARBITRUM_RPC or "localhost" in ARBITRUM_RPC
    candidates = [ARBITRUM_RPC]
    if not _proxy_rpc:
        candidates += [u for u in _ARBITRUM_RPC_FALLBACKS if u != ARBITRUM_RPC]
    last_exc = None
...
def cmd_prime_activate(amount: float = None, execute: bool = False):
    ...
    if required is not None and projected_in_acct < required:
        ...
        print("  Deposit more PRIME first: arbprime prime-activate --amount <N> --execute")
        return
...
def main():
    ...
    if cmd == "pool-info":
        ...
        # no-op — propagate it to the exit code (parity with degenprime).
