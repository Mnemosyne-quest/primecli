...
Usage:
  ...
  python3 deltaprime.py equity [--json] [--account 0x..]   (net equity = total value - debt + unclaimed rewards; read-only)
...
#   3. DELTAPRIME_PRIVATE_KEY env var            -> raw 0x… key
#   4. DELTAPRIME_KEY_FILE env var               -> path to a file containing the 0x key
#   5. DELTAPRIME_ENV_FILE + DELTAPRIME_KEY_VAR  -> read <var> from <file>
#   6. DELTAPRIME_AGENT env var                  -> AGENTS[<agent>]
# If none resolve, fail closed (no silent default key).
#
# To add another wallet: add a row to AGENTS, export DELTAPRIME_PRIVATE_KEY, or pass --key.
...
PARASWAP_EXECUTORS = {
    ...
    # Velora rotation executor, seen live 2026-08-12 (parakletos-2 VIRTUAL/ETH
    # converge). Verified ACCEPTED on-chain: the same address appears in the
    # confirmed swap txs of that tick (status 1) — the facet whitelists it. Added
    # so the "not in the KNOWN whitelist" warning stops firing for a known-valid
    # executor (it was a red herring in the 18:31 UK increase-failure post-mortem).
...
# is the parent of DegenPrime's, and every pool function this tool calls is in this
# subset. Previously deltaprime fetched per-pool ABIs from Snowtrace (5 API hits per
...
# (including Avalanche C-chain and Base). aggregate3(Call3[]) batches read-only calls
# into one eth_call; allowFailure=true per-call so a single revert returns success=false
# for that leg instead of blowing up the whole batch. Used to collapse per-pool /
# per-market fan-out loops (cmd_pool_info("all"), gather_lending, gather_gmx, my-positions)
# from N RPCs into 1. Same address on both chains; address verified on chainlist.org and
...
def _tx_gas_price(w3) -> int:
    ...
    current base fee with a 1 gwei floor. NOTE: _tx_gas_price is NOT used for tx building;
...
def resolve_private_key():
    ...
    raise RuntimeError(
        ...
        "DELTAPRIME_PRIVATE_KEY (raw 0x... key), DELTAPRIME_KEY_FILE (path to a file "
        "with the key), or DELTAPRIME_ENV_FILE + DELTAPRIME_KEY_VAR."
    )
...
# Every DeltaPrime lending Pool enforces an absolute borrow ceiling via the public view
...
# 0xe5739c7e). Verified 0.925 (92.5%) on every Avalanche pool 2026-07-18 — but READ IT
# LIVE, never assume: this constant is only the fallback for when the call reverts or a
# pool predates the getter, so a borrow-sizing caller can always stay below a real cap.
...
def cmd_my_positions():
    ...
    # Wallet AVAX — native gas. Suppress the line when the balance is effectively zero
    # (sub-nanowei dust) so a clean readout doesn't carry a noisy `AVAX: 0.000000`.
...
def _log_fund_flow(account_addr: str, symbol: str, amount: float, receipt,
    ...
    try:
        ...
        _flowledger.append_flow("avalanche", account_addr, rec)
...
def get_account_equity(account_addr: str = None) -> dict:
    ...
    getTotalValue/getDebt reads and gather_sjoe for unclaimed sJOE rewards (paid in
    USDC, so pending_raw is already USD). `account_addr` targets a specific Prime
    Account; when None it resolves the one owned by the --as / --owner wallet.
    ...
    base = {
        "agent": _SELECTED_AGENT, "chain": "avalanche", "protocol": "DeltaPrime",
    ...
    try:
        ...
        sjoe = gather_sjoe(account, w3)
        # sJOE rewards are paid in USDC (6 decimals) -> pending_raw is already USD.
        rewards = sjoe["pending_raw"] / 10 ** SJOE_REWARD["decimals"]
...
def cmd_equity(as_json: bool = False, account_addr: str = None):
    ...
    print(f"  Rewards:       ${rw:,.2f}  (unclaimed sJOE, paid in USDC)")
...
def _estimate_gmx_execution_fee(w3, is_deposit: bool, buffer_mult: float = 2.0):
    ...
    # 1 gwei is ~100× current Avalanche gas (0.01 gwei) and keeps the upfront requirement low.
...
def cmd_gmx_deposit(market: str, amount: float, is_long: bool | None = None,
    ...
    # Pre-check: EOA must hold the execution fee + tx gas (~0.0004 AVAX floor).
    ...
    if eoa_balance < min_needed:
        print(f"✗ EOA balance {eoa_balance / 1e18:.6f} AVAX is below the minimum needed "
              f"({min_needed / 1e18:.6f} AVAX: {exec_fee / 1e18:.6f} exec fee + {gas_buf / 1e18:.6f} tx gas).")
        print("  Transfer more AVAX to the EOA before retrying.")
        return
...
def gather_sjoe(account, w3=None):
    ...
    Multicall: both reads batch into one eth_call. `w3` is optional for backward compat;
    omit it and the legacy 2-RPC path runs. cmd_defi always passes it for the win."""
    if w3 is not None:
        pa_cs = account.address
        legs = [
            ("joeBalanceInSJoe", account.encode_abi("joeBalanceInSJoe", args=[])),
            ("rewardsInSJoe", account.encode_abi("rewardsInSJoe", args=[])),
        ]
        results = multicall(w3, [(pa_cs, bytes.fromhex(d[2:])) for _, d in legs])
        staked = w3.codec.decode(["uint256"], results[0][1])[0] if results[0][0] else 0
        pending = w3.codec.decode(["uint256"], results[1][1])[0] if results[1][0] else 0
    else:
...
def cmd_sjoe_stake(amount: float, execute: bool = False):
    ...
    amount_wei = to_wei_units(amount, SJOE_JOE["decimals"])
    ...
    receipt = _sign_and_send(w3, acct, tx, f"sJOE stake {amount} JOE", fallback_gas=3000000)
    ok = receipt["status"] == 1
...
def cmd_sjoe_unstake(amount: float, execute: bool = False):
    ...
    receipt = _sign_and_send(w3, acct, tx, "sJOE unstake", fallback_gas=3000000)
    ok = receipt["status"] == 1
...
def cmd_sjoe_claim(execute: bool = False):
    ...
    receipt = _sign_and_send(w3, acct, tx, "sJOE claim", fallback_gas=3000000)
    ok = receipt["status"] == 1
...
BRIDGE_CHAIN = {
    ...
    "arb": {"rpc": "https://arb1.arbitrum.io/rpc", "chain_id": 42161,
            ...
            "bridge_target": "0x3De81CE90f5A27C5E6A5aDb04b54ABA488a6d14E",
            "explorer": "https://arbiscan.io/tx", "native": "ETH"},
}

BRIDGE_ADAPTER_PARAMS = struct.pack('>H', 1) + struct.pack('>I', 200000).rjust(32, b'\x00')
...
def cmd_prime_bridge(from_chain: str = "arb", amount: float = None, execute: bool = False):
    """Bridge PRIME between Avalanche and Arbitrum via LayerZero OFT.
    Default from_chain='arb' for deltaprime (Arbitrum -> Avalanche)."""
    if amount is None:
        print("Usage: deltaprime prime-bridge --from avax|arb --amount N [--execute]")
        return
    ...
    local_acct = get_account()
    wallet = Web3.to_checksum_address(local_acct.address)
    ...
    ep = w3.eth.contract(address=Web3.to_checksum_address(src_cfg["lz_endpoint"]),
        abi=json.dumps([{"inputs":[{"name":"_dstChainId","type":"uint16"},{"name":"_userApplication","type":"address"},
            {"name":"_payload","type":"bytes"},{"name":"_payInZRO","type":"bool"},{"name":"_adapterParams","type":"bytes"}],
            "name":"estimateFees","outputs":[{"name":"","type":"uint256"},{"name":"","type":"uint256"}],
            "stateMutability":"view","type":"function"}]))
    dst_addr_b32 = bytes.fromhex(wallet.lower()[2:].rjust(64, "0"))
    payload = dst_addr_b32 + amount_wei.to_bytes(32, 'big')
    native_fee, _ = ep.functions.estimateFees(
        dst_cfg["lz_chain_id"], Web3.to_checksum_address(src_cfg["bridge_target"]),
        payload, False, BRIDGE_ADAPTER_PARAMS).call()
    ...
    cur = allow_c.functions.allowance(wallet, bridge_target).call()
    if cur < amount_wei:
        print(f"  Approving {amount} PRIME...")
        app_c = w3.eth.contract(address=Web3.to_checksum_address(src_cfg["prime_token"]),
            abi=json.dumps([{"constant": False, "inputs": [{"name": "_spender", "type": "address"},
        ...
        atx = app_c.functions.approve(bridge_target, amount_wei).build_transaction(
            {"from": wallet, "gas": 100000, "nonce": w3.eth.get_transaction_count(wallet),
             "chainId": src_chain_id})
        _set_gas_price_for(src_chain_id, w3, atx)
        signed = local_acct.sign_transaction(atx)
        w3.eth.wait_for_transaction_receipt(w3.eth.send_raw_transaction(signed.raw_transaction), timeout=120)
        import time; time.sleep(2)

    # 2. sendFrom
    zero = "0x0000000000000000000000000000000000000000"
    call_params = (wallet, zero, BRIDGE_ADAPTER_PARAMS)
    calldata_hex = "695ef6bf" + abi_encode(
    ...
    receipt = _sign_and_send(w3, local_acct, tx, "Bridge", timeout=300, fallback_gas=500000,
...
def gather_defi() -> dict:
    ...
    PRIME tier, plus the EOA's own pool deposits surfaced as a Savings group), each of which only does eth_calls. Empty groups are omitted. total_usd /
